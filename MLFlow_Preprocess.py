...
#2022-11-16 - Fernando Timoteo Fernandes
class Iterative_Imputer(BaseEstimator, TransformerMixin):
    """
    2022-11-16 - Fernando Timoteo Fernandes
    Imputes numerical and categorical using library fancyimpute.
      TODO: Upgrade to scikit-learn IterativeImputer when scikit-learn version 1+ supported by pyod library
      Args:
        strategy: string , all possible values {'mice'}
        target: string , name of the target variable
  """

    #Set attributes with parameters
    def __init__(self, strategy, target_variable):
        self.strategy = strategy #will only receive "mice" - IterativeImputer https://scikit-learn.org/stable/modules/generated/sklearn.impute.IterativeImputer.html#sklearn.impute.IterativeImputer
        self.target = target_variable

    #Creates a copy of data dataset 
    def fit(self, dataset, y=None):  #
        data = dataset.copy()

        #creates an attribute with only numeric columns
        self.numeric_columns = (
            data.drop(self.target, axis=1)
                .select_dtypes(include=["float64", "int64"])
                .columns
    ...
    #Apply transformation to the dataset using MICE (IterativeImputer)
    def transform(self, dataset, y=None):
        data = dataset.copy()
        
        if self.strategy != "ignore":
            mice_imputer = IterativeImputer() #Creates an IterativeImputer (MICE) from scikit-learn
            
            np_array_transformed_mice = mice_imputer.fit_transform(data)
            df_mice = pd.DataFrame(np_array_transformed_mice, index=data.index, columns=data.columns)
            data = df_mice
...
def Preprocess_Path_One(
  ...
  """
    ...
    #Fernando Timoteo Fernandes - 2022-11-16
    if(numeric_imputation_strategy == "mice"):
        print("trying customized MICE imputer...")
        imputer = Iterative_Imputer(strategy=numeric_imputation_strategy,
                                    target_variable=target_variable,)
    ...
    if test_data is not None:
        print("start pipe.fit_transform on train and test data separately")
...
class ResamplerTransformer(BaseEstimator, TransformerMixin):
  ...
  """
    ...
    def __init__(self, resampling_method):
        ...
        if resampling_method == "smoteenn_maj":
            from imblearn.combine import SMOTEENN
            self.resampler = SMOTEENN(random_state=42, n_jobs=-1,enn=EditedNearestNeighbours(sampling_strategy='majority'))
